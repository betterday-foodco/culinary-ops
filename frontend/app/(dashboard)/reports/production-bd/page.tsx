...
function downloadCsv(rows: ProductionRow[], weekStart: string) {
  const headers = ['Qty', 'Diet', 'Dish Name', 'SKU'];
  const csvRows = [
    headers.join(','),
    ...rows.map((r) =>
  ...
  a.href = url;
  a.download = `production-report-${weekStart || 'all'}.csv`;
  a.click();
...
export default function ProductionReportPage() {
  ...
  async function load(w?: string) {
    setLoading(true);
    try {
      const data = (await api.bdGetProductionReport(w || undefined)) as ProductionReport;
      setRows(data.rows ?? []);
    ...
    <div className="min-h-screen bg-[#FAEBDA] p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-[#003141]">Production Report</h1>
            {weekStart && (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#003141] text-white">
                    ...
                    <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide">Dish Name</th>
